...
.piano-container {
  ...
  border-radius: 8px;
  /* Permet le défilement horizontal si le clavier est encore trop large */
  overflow-x: auto;
}
...
.piano-keyboard {
  display: flex;
  background: #000;
  /* On s'assure que le clavier ne se réduit pas en dessous de sa taille minimale */
  flex-shrink: 0;
}
...
.piano-key.white {
  /* AJUSTEMENT : Largeur des touches blanches réduite */
  width: 44px;
  height: 180px;
...
.piano-key.black {
  position: absolute;
  top: 0;
  /* AJUSTEMENT : Position et largeur des touches noires adaptées */
  right: -14px;
  width: 28px;
  height: 110px;
